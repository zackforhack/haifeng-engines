...
export default async function ConfigurationFacetPage({ params }: Props) {
  ...
  const jsonLd = [
    { '@context': 'https://schema.org', '@type': 'CollectionPage', name: h1, url: `${BASE}/engines/configuration/${config}`, description: intro, mainEntity: { '@type': 'ItemList', numberOfItems: engines.length } },
  ...
  return (
    <>
      ...
      <FacetHub facetLabel={cfg.label} h1={h1} intro={intro} engines={engines} siblings={siblings} siblingHeading="Browse by configuration" />
    </>
  )
}
