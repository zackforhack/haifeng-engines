// Attach the compressed VMAN 2026 catalog (15.2MB -> 7.6MB) as a downloadable
// brochure on every VMAN engine page. Idempotent.
import { readFileSync } from 'fs'
import { createClient } from '@supabase/supabase-js'

const supabase = createClient('https://ntrysdovwnbegxtjsqkz.supabase.co', process.env.SUPABASE_SERVICE_KEY)
const LOCAL_PDF = '/tmp/vman-min.pdf'
const STORAGE = 'vman/vman-diesel-engine-catalog-2026.pdf'

const { data: engines } = await supabase.from('engines').select('id').eq('brand', 'VMAN')
console.log(`VMAN engines: ${engines.length}`)
...
const rows = engines.filter((e) => !have.has(e.id)).map((e) => ({
  engine_id: e.id, type: 'brochure', label: 'VMAN Diesel Engine Catalog 2026',
