...
const TMP_DIR = path.join(os.tmpdir(), 'yanmar-current-gap-spec-sheets-2026-08')
...
const records = [
  {
    slug: 'yanmar-3tnm68-hge',
    model: '3TNM68-HGE',
    sourceUrl: 'https://yanmarengines.com/wp-content/uploads/2020/09/3TNM68-HGE.pdf',
    storagePath: 'yanmar/official-spec-sheets/3tnm68-hge.pdf',
    label: 'Yanmar 3TNM68-HGE Spec Sheet',
    requiredTokens: ['3TNM68-HGE', 'YANMAR', 'Dimensions', 'Performance Data'],
    rejectTokens: ['3TNM68-GGE', '3TNV70-GGE', '3TNM72-GGE'],
  },
  {
    slug: 'yanmar-3tnv70-gge',
    model: '3TNV70-GGE',
    sourceUrl: 'https://yanmarengines.com/wp-content/uploads/2020/09/3TNV70-GGE.pdf',
    storagePath: 'yanmar/official-spec-sheets/3tnv70-gge.pdf',
    label: 'Yanmar 3TNV70-GGE Spec Sheet',
    requiredTokens: ['3TNV70-GGE', 'YANMAR', 'Dimensions', 'Performance Data'],
    rejectTokens: ['3TNV70-HGE', '3TNM68-HGE', '3TNM72-GGE'],
  },
]
...
function assertStillMissingExclusive() {
  ...
  const missingSlugs = new Set((report.groups?.Yanmar ?? []).map((row) => row.slug))
...
console.log(`${APPLY ? 'APPLY' : 'DRY RUN'}: ${records.length} Yanmar current-gap spec sheets`)

for (const record of records) {
  const engine = engineBySlug.get(record.slug)
  if (!engine) throw new Error(`Missing engine row: ${record.slug}`)
  if (engine.brand !== 'Yanmar' || normalize(engine.model) !== normalize(record.model)) {
