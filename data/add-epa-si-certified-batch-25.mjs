// Add Cummins' current EPA-certified large 60 Hz natural-gas packages.
...
const common = {
  ...
  certifications: ['U.S. EPA NSPS', 'U.S. EPA MOH'],
  cylinders: 16,
  displacement_l: 60.2,
...
const records = [
  ...
  },
  {
    ...
    model: 'C1350N6',
    standby_power_kwe_60hz: 1350,
    standby_power_kva_60hz: 1688,
  },
  {
    ...common,
    slug: 'cummins-c1400n6c',
    model: 'C1400N6C',
    prime_power_kwe_60hz: 1400,
    prime_power_kva_60hz: 1750,
  },
].map((record) => ({
  ...record,
  description:
    `Cummins ${record.model} is a 60 Hz natural-gas generator set using `
    + 'the 60.2 L QSK60G lean-burn V16 engine at 1800 RPM. Cummins lists '
    + `${record.standby_power_kwe_60hz
      ? `${record.standby_power_kwe_60hz} kWe standby`
      : `${record.prime_power_kwe_60hz} kWe continuous`} output and `
    + 'EPA NSPS/MOH certification in its official North American range.',
}))
...
console.table(records.map((record) => ({
  ...
  slug: record.slug,
  duty: record.standby_power_kwe_60hz ? 'standby' : 'continuous',
  kwe: record.standby_power_kwe_60hz ?? record.prime_power_kwe_60hz,
})))
if (!apply) {
  console.log(
