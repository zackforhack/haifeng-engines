...
const TMP_DIR = path.join(os.tmpdir(), 'emean-weichai-crawl-2026-08')
...
const CATEGORY_PAGES = [
  '/WEICHAI-16KW-880KW/',
  '/WEICHAI-16KW-880KW/page_2.html',
  '/WEICHAI-16KW-880KW/page_3.html',
  '/WEICHAI-16KW-880KW/page_4.html',
  '/WEICHAI-16KW-880KW/page_5.html',
  '/WEICHAI-16KW-880KW/page_6.html',
  '/WEICHAI-800KW-1600KW/',
  '/WEICHAI-800KW-1600KW/page_2.html',
  '/WEICHAI-800KW-1600KW/page_3.html',
]
...
function extractProductUrls() {
  ...
  for (const categoryPath of CATEGORY_PAGES) {
    ...
    const pattern = /href=(["'])(\/WEICHAI-(?:16KW-880KW|800KW-1600KW)\/[^"']+?\.html)\1/g
...
function extractEngineDatasheet(productUrl, html) {
  ...
  if (!/engine/i.test(title) || !/weich/i.test(title)) return null
...
function buildRecords(candidates, missingRows) {
  ...
  for (const candidate of candidates) {
    ...
    records.push({
      ...
      storagePath:
        `weichai/emean-engine-datasheets/${storageSegment(row.model)}.pdf`,
    })
  }
...
console.log(`Crawled ${productUrls.length} Emean Weichai product page URL(s).`)
...
console.log(
  `${APPLY ? 'APPLY' : 'DRY RUN'}: `
  + `${records.length} current missing-exclusive Weichai matches from `
