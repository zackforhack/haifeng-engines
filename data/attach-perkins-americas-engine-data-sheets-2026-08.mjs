...
const TMP_DIR = path.join(os.tmpdir(), 'perkins-americas-engine-data-sheets-2026-08')
...
const records = [
  {
    slug: 'perkins-2806c-e18ttag7',
    model: '2806C-E18TTAG7',
    sourcePage:
      'https://americasgenerators.com/750-kw-triton-diesel-generator-tp-p750-t2-ul-tier-2-ul-listed/',
    sourceUrl:
      'https://americasgenerators.com/1-tp-spec-sheets/perkins-ul/tp-p750-t2-60-ul/2806c-e18ttag7.pdf',
    storagePath: 'perkins/americas-engine-data-sheets/2806c-e18ttag7.pdf',
    label: 'Perkins 2806C-E18TTAG7 Engine Data Sheet',
    requiredMarkers: ['Basic technical data'],
    rejectTokens: ['2806C-E18TTAG6', '2806A-E18TTAG7', '2806A-E18TTAG6'],
  },
  {
    slug: 'perkins-4008tag2',
    model: '4008TAG2',
    sourcePage:
      'https://americasgenerators.com/875-kw-triton-diesel-generator-tp-p875-t1-international-use/',
    sourceUrl:
      'https://americasgenerators.com/1-tp-spec-sheets/perkins-non-epa/60-hz/tp-p875-t1-60/4008tag2.pdf',
    storagePath: 'perkins/americas-engine-data-sheets/4008tag2.pdf',
    label: 'Perkins 4008TAG2 Engine Data Sheet',
    requiredMarkers: ['Diesel Engine - Electro Unit', 'Specification'],
    rejectTokens: ['4008TAG1', '4008TAG2A', '4008TAG3'],
  },
]
...
console.log(`${APPLY ? 'APPLY' : 'DRY RUN'}: ${records.length} Perkins engine data sheets`)

for (const record of records) {
  const engine = engineBySlug.get(record.slug)
  if (!engine) throw new Error(`Missing engine row: ${record.slug}`)
  if (engine.brand !== 'Perkins' || normalize(engine.model) !== normalize(record.model)) {
